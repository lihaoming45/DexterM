...
# from grasping_lhm.utils.vis_utils import colors
...
class HandModel:
        ...
        def build_mesh_recurse(body):
            if(len(body.link.visuals) > 0):
                ...
                for visual in body.link.visuals:
                    ...
                    elif visual.geom_type == "mesh" or visual.geom_type is None:
    ...
    def get_trimesh_data(self, i, opacity=0.5, color='grey'):
        """
        ...
        for link_name in self.mesh:
            ...
            f = self.mesh[link_name]['faces'].detach().cpu().numpy()
            # mesh_i = tm.base.Trimesh(vertices=v, faces=f,vertex_colors=self.n_surface_points*colors[color])
            mesh_i=tm.Trimesh(vertices=v, faces=f)
            data.append(mesh_i)
