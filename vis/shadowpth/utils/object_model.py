...
class ObjectModel:
    ...
    def __init__(self, data_root_path, batch_size_each, num_samples=2048, device="cuda"):
        """
    ...
    def initialize(self, object_code_list):
        ...
        self.surface_normals_tensor = []

        for object_code in object_code_list:
            self.object_scale_tensor.append(self.scale_choice[
                                                torch.randint(0, self.scale_choice.shape[0], (self.batch_size_each,),
                                                              device=self.device)])
            self.object_mesh_list.append(
                tm.load(os.path.join(self.data_root_path, object_code, "coacd", "decomposed.obj"), force="mesh",
                        process=False))
            ...
            if self.num_samples != 0:
                ...
                dense_point_cloud, dense_normals = pytorch3d.ops.sample_points_from_meshes(mesh,
                                                                                           num_samples=100 * self.num_samples,
                                                                                           return_normals=True)
                surface_points, idx = pytorch3d.ops.sample_farthest_points(dense_point_cloud,
                                                                           K=self.num_samples)  # [0][0]
                surface_points.to(dtype=float, device=self.device)
                surface_normals = dense_normals[:, idx[0], :]
                surface_normals.to(dtype=float, device=self.device)

                self.surface_points_tensor.append(surface_points)
                self.surface_normals_tensor.append(surface_normals)
        ...
        if self.num_samples != 0:
            ...
            self.surface_normals_tensor = torch.stack(self.surface_normals_tensor, dim=0).repeat_interleave(
                self.batch_size_each, dim=0)  # (n_objects * batch_size_each, num_samples, 3)
    ...
    def get_plotly_data(self, i, color='lightgreen', opacity=0.5, pose=None):
        ...
        data = go.Mesh3d(x=vertices[:, 0], y=vertices[:, 1], z=vertices[:, 2], i=mesh.faces[:, 0], j=mesh.faces[:, 1],
                         k=mesh.faces[:, 2], color=color, opacity=opacity)
        return [data]

    def get_trimesh_data(self, i):
        model_index = i // self.batch_size_each
        model_scale = self.object_scale_tensor[model_index, i % self.batch_size_each].detach().cpu().numpy()
        mesh = self.object_mesh_list[model_index].copy()
        mesh.vertices *= model_scale
        return mesh

    def get_piece_meshlist(self, i):
        object_code = self.object_code_list[i]
        piece_paths = glob.glob(os.path.join(self.data_root_path, object_code, "coacd", "coacd_convex*.obj"))
        piece_mesh_list = [tm.load(p_path, force="mesh", process=False) for p_path in piece_paths]

        model_index = i // self.batch_size_each
        model_scale = self.object_scale_tensor[model_index, i % self.batch_size_each].detach().cpu().numpy()
        for m_i in piece_mesh_list:
            m_i.vertices *= model_scale

        return piece_mesh_list

    def get_surface_points(self, i):
        model_index = i // self.batch_size_each
        model_scale = self.object_scale_tensor[model_index, i % self.batch_size_each].detach().cpu()

        surface_points = self.surface_points_tensor[model_index].detach().cpu() * model_scale  # (1,n_samples,3)
        surface_normals = self.surface_normals_tensor[model_index].detach().cpu()  # (1,n_samples,3)
